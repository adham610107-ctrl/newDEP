// ==========================================
// 1. GOOGLE SHEETS & PWA CONFIGURATION
// ==========================================
...
function getOrCreateDeviceId() {
    ...
    if (!deviceId) { deviceId = 'dev_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now(); localStorage.setItem('adham_pro_device_id', deviceId); }
    return deviceId;
}

let deferredPrompt;
window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); deferredPrompt = e; const btn = document.getElementById('install-app-btn'); if (btn) btn.classList.remove('hidden'); });
document.addEventListener('DOMContentLoaded', () => { const btn = document.getElementById('install-app-btn'); if (btn) { btn.addEventListener('click', async () => { if (deferredPrompt) { deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if (outcome === 'accepted') btn.classList.add('hidden'); deferredPrompt = null; } }); } });
...
function copyCard() {
    ...
    navigator.clipboard.writeText(cardText).then(() => { alert("💳 Karta raqami nusxalandi: " + cardText + "\nEndi ilovangizga o'tib to'lovni amalga oshirishingiz mumkin. Qo'llab-quvvatlaganingiz uchun rahmat!"); }).catch(err => console.error(err));
}
...
document.addEventListener('keydown', function(e) {
    ...
    if(e.ctrlKey && e.keyCode === 67) { e.preventDefault(); alert("⚠️ Ko'chirish (nusxalash) qat'iyan taqiqlangan!"); return false; }
});
...
document.addEventListener("visibilitychange", () => {
    ...
    if (testScreen && !testScreen.classList.contains("hidden") && document.hidden) {
        cheatWarnings++;
        if (cheatWarnings >= 3) { alert("❌ DIQQAT! 3 marta oynadan chiqdingiz. Intellektual sessiya avtomatik yakunlandi!"); finishExam(true); } 
        else { alert(`⚠️ OGOHLANTIRISH (${cheatWarnings}/3)!\n\nBoshqa oynaga o'tish (Ko'chirish) qat'iyan taqiqlanadi!`); }
    }
});
...
async function checkAdminBlock() {
    ...
    try {
        ...
        if (result.blocked) { alert("❌ DIQQAT: Tizim ma'muriyati (Admin) tomonidan bloklangansiz!"); localStorage.removeItem('pro_exam_auth'); localStorage.removeItem('pro_exam_name'); location.reload(); }
    } catch (e) {}
}

async function authenticateUser() {
    const loginVal = document.getElementById('auth-login').value.trim(); const passVal = document.getElementById('auth-password').value.trim(); const keygenVal = document.getElementById('auth-keygen').value.trim(); const errorEl = document.getElementById('auth-error'); const btn = document.getElementById('btn-auth');
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) { localStorage.setItem('pro_exam_auth', 'true'); localStorage.setItem('pro_exam_name', result.name || loginVal); document.getElementById('student-name').value = result.name || loginVal; switchScreen('auth-screen', 'welcome-screen'); setInterval(checkAdminBlock, 45000); } 
        else { errorEl.innerText = result.message; errorEl.classList.remove('hidden'); }
    } catch (e) { errorEl.innerText = "Tarmoqda xatolik."; errorEl.classList.remove('hidden'); } 
    finally { btn.innerText = "Kirishni Tasdiqlash 🔒"; btn.disabled = false; }
}
...
function playFeedback(type) {
    ...
    if(type === 'correct') { osc.type = 'sine'; osc.frequency.setValueAtTime(600, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(1200, audioCtx.currentTime + 0.1); gain.gain.setValueAtTime(0.3, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.1); osc.start(); osc.stop(audioCtx.currentTime + 0.1); if("vibrate" in navigator) navigator.vibrate(50); } 
    else { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(300, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(100, audioCtx.currentTime + 0.2); gain.gain.setValueAtTime(0.3, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.2); osc.start(); osc.stop(audioCtx.currentTime + 0.2); if("vibrate" in navigator) navigator.vibrate([150, 100, 150]); }
}
...
function speakQuestion(idx) {
    if('speechSynthesis' in window) { window.speechSynthesis.cancel(); let text = currentTest[idx].q; let msg = new SpeechSynthesisUtterance(text); msg.lang = 'uz-UZ'; msg.rate = 0.9; window.speechSynthesis.speak(msg); } 
...
function showComboBadge() { const badge = document.getElementById('combo-badge'); badge.innerText = `COMBO x${comboCount} 🔥`; badge.classList.remove('hidden'); badge.style.animation = 'none'; void badge.offsetWidth; badge.style.animation = 'comboPop 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards'; setTimeout(() => badge.classList.add('hidden'), 2000); }
function showHackerBadge() { const badge = document.getElementById('hacker-badge'); badge.classList.remove('hidden'); badge.style.animation = 'none'; void badge.offsetWidth; badge.style.animation = 'comboPop 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards'; setTimeout(() => badge.classList.add('hidden'), 3000); }

function updateDailyStreak() {
    let today = new Date().toDateString(); let lastDate = localStorage.getItem('adham_last_date'); let streak = parseInt(localStorage.getItem('adham_streak')) || 0;
    if (lastDate !== today) { let yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1); if (lastDate === yesterday.toDateString()) { streak++; } else if (lastDate) { streak = 1; } else { streak = 1; } localStorage.setItem('adham_last_date', today); localStorage.setItem('adham_streak', streak); }
    document.getElementById('streak-count').innerText = streak;
}

// Time-based AI Greeting
function updateGreeting() {
    const hour = new Date().getHours(); let text = "Xayrli tun";
    if(hour >= 5 && hour < 12) text = "Xayrli tong"; else if(hour >= 12 && hour < 18) text = "Xayrli kun"; else if(hour >= 18 && hour < 22) text = "Xayrli kech";
    const el = document.getElementById('greeting-text'); if(el) el.innerText = text;
}
...
let pendingSubject = null; let pendingLevelQs = []; let testType = null; let testModeName = ""; let diffTime = 900; let orderMode = 'random'; let isExamMode = false; let menuReturns = 0; let isAdminAdham = false;
...
async function loadData() {
    ...
    for (const f of files) {
        try {
            ...
            data.forEach(q => {
                let opts = q.options.filter(o => o !== null && o !== undefined && o.toString().trim() !== ''); let uniqueOpts = [...new Set(opts)]; let correctText = q.options[q.answer]; if(uniqueOpts.length === 3) uniqueOpts.push("Barcha javoblar to'g'ri"); 
...
window.onload = () => {
    loadData(); updateGreeting();
    const isAuth = localStorage.getItem('pro_exam_auth');
    if (isAuth === 'true') { document.getElementById('student-name').value = localStorage.getItem('pro_exam_name') || ''; switchScreen('auth-screen', 'welcome-screen'); checkAdminBlock(); setInterval(checkAdminBlock, 45000); }
    const slider = document.getElementById('theme-slider');
...
function switchScreen(hideId, showId) { forceCloseAllModals(); document.querySelectorAll('.screen').forEach(s => { s.classList.remove('active'); s.classList.add('hidden'); }); document.getElementById(showId).classList.remove('hidden'); document.getElementById(showId).classList.add('active'); }

function handleLogin() {
    const name = document.getElementById('student-name').value.trim(); if(name.length < 2) return alert("Ismingizni kiriting!");
    isAdminAdham = (name.toLowerCase() === 'adham' || name.toLowerCase() === 'admin'); if(isAdminAdham) alert("Assalomu alaykum, Muhtaram Creator (Admin)!");
...
function goHome() { 
    clearInterval(timerInterval); forceCloseAllModals(); document.getElementById('exit-test-btn').classList.add('hidden'); document.getElementById('exam-timer').classList.add('hidden'); document.body.classList.remove('boss-fight-mode'); if('speechSynthesis' in window) window.speechSynthesis.cancel();
    cheatWarnings = 0; comboCount = 0; hackerStreak = 0; totalErrorsInTest = 0; document.getElementById('restart-mini-btn').classList.add('hidden'); 
...
function updateDashboardStats() {
    stats.learned = [...new Set(stats.learned)]; stats.errors = [...new Set(stats.errors)]; stats.errors = stats.errors.filter(id => !stats.learned.includes(id)); 
...
function updateDashboardRentgen() {
    ...
    if(barsContainer) {
        ...
        subjects.forEach(sub => {
            let subQs = bank.filter(q => q.subject === sub); if(subQs.length === 0) return;
            let subCorrect = subQs.filter(q => stats.learned.includes(q.id)).length; let subPercent = Math.round((subCorrect / subQs.length) * 100);
            let color = subPercent >= 90 ? 'var(--success)' : subPercent >= 60 ? 'var(--warning)' : 'var(--error)'; let msg = subPercent >= 90 ? '(Ajoyib!)' : subPercent >= 60 ? '(Yaxshi)' : '(Kuchsiz)'; let subNameFormatted = subjectNames[sub] || sub;
            barsContainer.innerHTML += `<div class="rentgen-item"><div class="rentgen-label"><span>${subNameFormatted} ${msg}</span><span style="color:${color}">${subPercent}%</span></div><div class="rentgen-bar-bg"><div class="rentgen-bar-fill" style="width: ${subPercent}%; background: ${color};"></div></div></div>`;
        });
    }
    drawTradingChart();
}
...
function drawTradingChart() {
    const container = document.getElementById('trade-history'); if(!container) return;
    let total = bank.length || 1; let greenH = (stats.learned.length / total) * 50; let redH = (stats.errors.length / total) * 50; 
    if (greenH < 2 && stats.learned.length > 0) greenH = 2; if (redH < 2 && stats.errors.length > 0) redH = 2;
    let bars = ''; const numBars = 20; 
    for(let i=0; i<numBars; i++) {
        let randG = 0, randR = 0;
        if (i === numBars - 1) { randG = greenH; randR = redH; } 
        else { let progress = (i + 1) / numBars; let noise = Math.random() * 8 - 4; randG = Math.max(0.5, greenH * progress + noise); randR = Math.max(0.5, redH * progress - noise); }
        bars += `<div style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; padding: 0 1px;"><div style="height:50%; width:100%; display:flex; align-items:flex-end; justify-content:center; padding-bottom:1px;"><div style="width:70%; max-width:12px; background:linear-gradient(to top, #28A745, #30D158); height:${randG}%; border-radius:2px 2px 0 0; box-shadow:0 0 5px rgba(48,209,88,0.4); transition: 1s;"></div></div><div style="height:50%; width:100%; display:flex; align-items:flex-start; justify-content:center; padding-top:1px;"><div style="width:70%; max-width:12px; background:linear-gradient(to bottom, #FF3B30, #FF453A); height:${randR}%; border-radius:0 0 2px 2px; box-shadow:0 0 5px rgba(255,69,58,0.4); transition: 1s;"></div></div></div>`;
    }
    ...
    subjectsInTest.forEach(sub => {
        ...
        barsContainer.innerHTML += `
            <div class="rentgen-item">
                <div class="rentgen-label"><span>${subNameFormatted} ${subMsg}</span><span style="color:${barColor}">${subPercent}%</span></div>
                <div class="rentgen-bar-bg"><div class="rentgen-bar-fill" style="width: ${subPercent}%; background: ${barColor};"></div></div>
            </div>`;
    });
