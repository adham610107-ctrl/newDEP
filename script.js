...
// ==========================================
// SECURITY: ANTI-CHEAT & KEYBOARD PROTECTION
// ==========================================
function copyCard() {
...
async function checkAdminBlock() {
    ...
    try {
        const response = await fetch(GOOGLE_SCRIPT_URL, {
            method: 'POST', body: JSON.stringify({ action: "check_block", login: savedName })
        });
...
async function authenticateUser() {
    ...
    try {
        const response = await fetch(GOOGLE_SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify({ login: loginVal, password: passVal, keygen: keygenVal, deviceId: getOrCreateDeviceId() })
        });
        const result = await response.json();
        if (result.success) {
...
// ==========================================
// GOOGLE VIBE: AUDIO, COMBO & ANIMATIONS
// ==========================================
...
function createParticles(event) {
    ...
    for (let i = 0; i < 15; i++) {
        ...
        let destX = x + (Math.random() - 0.5) * 140; let destY = y + (Math.random() - 0.5) * 140;
...
function updateDailyStreak() {
    ...
    let lastDate = localStorage.getItem('adham_last_date');
...
window.onload = () => {
    ...
    if (isAuth === 'true') {
        document.getElementById('student-name').value = localStorage.getItem('pro_exam_name') || '';
        switchScreen('auth-screen', 'welcome-screen');
        checkAdminBlock();
    }
...
function toggleTheme() { 
    if(document.body.classList.contains('dark-mode')) { document.body.classList.replace('dark-mode', 'light-mode'); localStorage.setItem('theme', 'light'); }
...
function handleLogin() {
    ...
    // EASTER EGG
    if(name.toLowerCase() === 'adham' || name.toLowerCase() === 'admin') {
        alert("Assalomu alaykum, Muhtaram Creator (Admin)!");
    }
...
function goHome() { 
    ...
    cheatWarnings = 0; comboCount = 0; hackerStreak = 0;
    switchScreen('test-screen', 'dashboard-screen'); 
    updateDashboardStats(); 

    // DONATE MODAL LOGIC (Har 3 marta asosiy menyuga qaytganda)
    menuReturns++;
    if(menuReturns % 3 === 0) {
        setTimeout(() => {
            document.getElementById('modal-donate').style.display = 'flex';
            confetti({ particleCount: 50, spread: 60, origin: { y: 0.6 }, colors: ['#FFD700', '#FFA500'] });
        }, 500);
    }
}
...
function prepareTest(type, modeName) {
    forceCloseAllModals(); if (type === 'errors' && stats.errors.length === 0) return alert("Xatolar topilmadi!");
    testType = type; testModeName = modeName; openSetup();
}
...
function applySetup() {
    forceCloseAllModals(); isExamMode = false; let pool = []; cheatWarnings = 0; comboCount = 0; hackerStreak = 0; document.body.classList.remove('boss-fight-mode');
...
function startExamMode() {
    forceCloseAllModals(); testType = 'exam'; testModeName = "IMTIHON MODE (Boss Fight)"; isExamMode = true; cheatWarnings = 0; comboCount = 0; hackerStreak = 0; let examQs = [];
    ...
    area.innerHTML = currentTest.map((q, idx) => `
        <div class="q-block ${idx === currentIndex ? 'active-q' : 'blurred-q'}" id="q-block-${idx}">
            ...
            <div class="options-box" id="opts-${idx}">
                ${q.options.map((opt, optIdx) => `<button class="option-btn" id="btn-${idx}-${optIdx}" onclick="checkAns(${idx}, ${optIdx}, event)" ${userAnswers[idx] ? 'disabled' : ''}>${opt}</button>`).join('')}
            </div>
        </div>
    `).join('');
...
function finishExam() {
    ...
    document.body.classList.remove('boss-fight-mode'); document.getElementById('boss-fight-warning').classList.add('hidden');
...
// RENTGEN TAHLIL VA SERTIFIKAT LOGIKASI
function showResult(correctCount) {
    ...
    if(percent >= 90) { 
        msg = "Muhtasham natija! Siz haqiqiy mutaxassissiz. 🏆"; color = "var(--success)"; 
        confetti({ particleCount: 200, spread: 90, origin: { y: 0.6 } });
        document.getElementById('cert-btn').style.display = 'inline-block';
    } else if(percent >= 70) { 
        msg = "Yaxshi ko'rsatkich, lekin Akademik cho'qqiga oz qoldi. 👍"; color = "var(--primary)"; 
        document.getElementById('cert-btn').style.display = 'none';
    } else if(percent >= 50) { 
        msg = "Qoniqarli, ammo intellektual salohiyatingiz bundan baland! 📚"; color = "var(--warning)"; 
        document.getElementById('cert-btn').style.display = 'none';
    } else { 
        msg = "Chuqur tahlil qiling va qayta urinib ko'ring! ⚠️"; color = "var(--error)"; 
        document.getElementById('cert-btn').style.display = 'none';
    }
    ...
    // Rentgen tahlilini yaratish
    generateRentgenAnalysis();

    forceCloseAllModals(); document.getElementById('modal-result').style.display = 'flex';
}

function generateRentgenAnalysis() {
        ...
        barsContainer.innerHTML += `
            <div class="rentgen-item">
                <div class="rentgen-label">
                    <span>${subNameFormatted} ${msg}</span>
                    <span style="color:${color}">${subPercent}%</span>
                </div>
                <div class="rentgen-bar-bg">
                    <div class="rentgen-bar-fill" style="width: ${subPercent}%; background: ${color};"></div>
                </div>
            </div>
        `;
    });
